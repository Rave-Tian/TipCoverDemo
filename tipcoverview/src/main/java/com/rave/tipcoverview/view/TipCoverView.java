package com.rave.tipcoverview.view;
 ...
 * ======================================================================
 */
public class TipCoverView extends FrameLayout {
    /**
     * 默认模糊边界的大小
     */
     ...
     * 用于保存高亮View的集合
     */
    private List<TipCoverHelper.ViewPosInfo> mViewRects;
    /**
     * HighLight对象
     */
    private TipCoverHelper mTipCoverHelper;
    /**
     * 打气筒
     */
     ...
     * @param myType
     */
    public void setMyType(TipCoverHelper.MyType myType) {
        this.myType = myType;
    }
    ...
    public TipCoverView(Context context, TipCoverHelper highLight, int maskColor, List<TipCoverHelper.ViewPosInfo> viewRects) {
        super(context);
        this.context = context;
        mTipCoverHelper = highLight;
    ...
    private void addViewForTip() {
        for (TipCoverHelper.ViewPosInfo viewPosInfo : mViewRects) {
            ...
            lp.bottomMargin = (int) viewPosInfo.marginInfo.bottomMargin + 50;
    ...
    private void buildMask() {
        ...
        if (isBlur)
            mPaint.setMaskFilter(new BlurMaskFilter(this.blurSize,
                    BlurMaskFilter.Blur.SOLID));
        mTipCoverHelper.updateInfo();

        for (TipCoverHelper.ViewPosInfo viewPosInfo : mViewRects) {
            if (viewPosInfo.needHightLight) {
     ...
     * @param radius
     */
    private void drawCircleBorder(Canvas canvas, TipCoverHelper.ViewPosInfo viewPosInfo, float circle_center1, float circle_center2, int radius) {
        Paint paint = new Paint();
        paint.reset();
        if (this.myType == TipCoverHelper.MyType.DASH_LINE) {
     ...
     * @param viewPosInfo
     */
    private void drawRectBorder(Canvas canvas, TipCoverHelper.ViewPosInfo viewPosInfo) {
        Paint paint = new Paint();
        paint.reset();
        if (this.myType == TipCoverHelper.MyType.DASH_LINE) {
            DashPathEffect pathEffect = new DashPathEffect(intervals, this.phase);
    ...
    private void updateTipPos() {
        for (int i = 0, n = getChildCount(); i < n; i++) {
            ...
            TipCoverHelper.ViewPosInfo viewPosInfo = mViewRects.get(i);
    ...
    private LayoutParams buildTipLayoutParams(View view,
                                              TipCoverHelper.ViewPosInfo viewPosInfo) {
